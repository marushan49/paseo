...
export const TESTING_ENGINE_INSTRUCTION = `For testing, whenever \`paseo_test\` is available it is the required first step for any UI, end-to-end, or "does it work" check, in every project: list the saved recipes, run a matching one, or send ad-hoc steps with \`goal\` steps for the parts you cannot script. It runs inside the daemon and returns only the verdict, which costs a fraction of the tokens of driving \`browser_*\` tools yourself. Use the \`browser_*\` tools only to debug a failed run, and after a new flow passes, save it as a recipe in paseo.json so it runs fully scripted next time.`;
